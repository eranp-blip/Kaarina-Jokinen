...
function buildApp(shadow){
  const app=h('div',{class:'app-frame'},[
    ...
    h('main',{class:'main'},[
      h('section',{class:'hero',id:'guide'},[
        ...
        h('div',{class:'actions'},[
          h('a',{class:'button primary',href:'#newsletter',text:'Sign up to the newsletter'}),
          h('a',{class:'button secondary',href:'#article',text:'Read the guide'})
        ])
      ]),
      h('section',{class:'grid',id:'article'},copy.sections.map(s=>h('article',{class:'card'},[
        h('h2',{text:s[0]}),
        h('p',{text:s[1]})
      ]))),
  ...
  form.addEventListener('submit',e=>{
    ...
    const leads=JSON.parse(localStorage.getItem('newsletter_leads')||'[]');
    leads.push({...data,createdAt:new Date().toISOString(),page:location.href});
    localStorage.setItem('newsletter_leads',JSON.stringify(leads));
    form.reset();
    app.querySelector('.notice').style.display='block';
  });
...
if(host){
  ...
  cssLink.href='assets_vjs/main-CYEVdsHA.css';
  closedShadow.appendChild(cssLink);
